...
module.exports = {
  ...
  commentCharacter: function() {
    var commentChar = utils.execSync('git config --get core.commentchar').trim();
  ...
  mergeState: function() {
    ...
    var currMergeHead = utils.execSync('git rev-parse MERGE_HEAD 2> #{File::NULL}').trim();
    if (currMergeHead !== 'MERGE_HEAD') {
      mergeState.mergeHead = currMergeHead;
    }

    var mergeMsgPath = this.gitDir(this.repoRoot()) + '/MERGE_MSG';
    if (fse.exists(mergeMsgPath)) {
      mergeState.mergeMsg = fse.readFileSync(mergeMsgPath).toString();
    }
    return mergeState;
  },
  ...
  cherryPickState: function() {
    ...
    var currCherryPickHead = utils.execSync('git rev-parse CHERRY_PICK_HEAD 2> #{File::NULL}').trim();
