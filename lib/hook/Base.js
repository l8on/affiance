...
function HookBase(config, context) {
  this.config = config.forHook(this.hookName(), context.hookScriptName);
  this.context = context;
}

HookBase.prototype.hookName = function() {
  if (!this._hookName) {
    this._hookName = path.basename(__filename, '.js');
  }
...
HookBase.prototype.flags = function() {
  return [].concat(this.config['flags']);
};
...
HookBase.prototype.isApplicable = function(filePath) {
  ...
  var includes = _.flatten([].concat(this.config['include'])).map(fileUtils.convertGlobToAbsolute);
  ...
  var excludes = _.flatten([].concat(this.config['include'])).map(fileUtils.convertGlobToAbsolute);
...
HookBase.prototype.checkForExecutable = function() {
  // If this hook doesn't require an executable, chill
