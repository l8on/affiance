...
HookContextPreCommit.prototype.isAmendment = function() {
  if (this._isAmendment === null) {
    ...
    var commandResult = utils.spawnSync('git', ['config', '--get-regexp', '"^alias\."', '"commit(\s.*)?\s--amend(\s|$)"']);
    var commandOutput = commandResult.stdout.toString();
    var aliasRegex = /alias\.([-\w]+)/g;
    var matches = null;

    while (matches = aliasRegex.exec(commandOutput)) {
