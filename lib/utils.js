...
var SUPPORTED_HOOK_CONFIG_NAMES = SUPPORTED_HOOK_TYPES.map(classify);
...
module.exports = {
  ...
  isInPath: function(commandName) {
    var paths = [gitRepo.repoRoot()].concat(process.env.PATH.split(path.delimiter));
    var exts = process.env.PATHEXT ? process.env.PATHEXT.split(';') : [''];
    ...
    for (var i in paths) {
      ...
      for (var j in exts) {
        var ext = exts[j];
        // var commandWithExt = commandName.toLowerCase().slice(-1 * ext.length) === ext.toLowerCase() ? commandName : commandName + ext;
        var commandFullPath = [path, commandWithExt].join('/');
