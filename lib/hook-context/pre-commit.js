...
HookContextPreCommit.prototype.isAmendment = function() {
  if (this._isAmendment === null) {
    ...
    var configCommand = 'git config --get-regexp "^alias\." "commit(\s.*)?\s--amend(\s|$)"';
    var commandResults = childProcess.execSync(configCommand).toString();
    var aliasRegex = /alias\.([-\w]+)/g;
    var  matches = null;

    while (matches = aliasRegex.exec(commandResults)) {
      var matchRegex = new RegExp('git(\\.exe)?\\s+' + matches[0]);
...
HookContextPreCommit.prototype.anyChanges = function() {
  var command = 'git status -z --untracked-files=no';
  var commandResults = childProcess.execSync('git status -z --untracked-files=no').toString().trim();
...
HookContextPreCommit.prototype.setupEnvironment = function() {
  ...
  if (!gitRepo.isInitialCommmit() && this.anyChanges()) {
...
HookContextPreCommit.prototype.cleanupEnvironment = function() {
  if (!(gitRepo.isInitialCommmit() || (this._stashAttempted && !this._changesStashed)) ) {
...
HookContextPreCommit.prototype.restoreWorkingTree = function() {
  var commandResult = utils.execSync('git stash pip --index --quiet');
  if (commandResult === false) {
    throw AffianceError.error(
      AffianceError.HookCleanupFailed,
      'Unable to cleanup working tree for commit-msg hook run'
...
HookContextPreCommit.prototype.storeModifiedTimes = function() {
  ...
  var unstagedFiles = gitRepo.modifiedFiles({staged: true});
...
HookContextPreCommit.prototype.restoreModifiedTimes = function() {
  for(var modifiedFilePath in this._modifiedTimes) {
    ...
    var mtime = this._modifiedTimes[modifiedFilePath];
