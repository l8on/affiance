...
var IGNORED_CONFIG_KEYS = ['skip'];

function HookSigner(hookName, config, context) {
  this.hookName = hookName;
  this.config = config;
  this.context = context;
}

HookSigner.prototype.hookPath = function() {
  if (!this._hookPath) {
    ...
    try {
      ...
      var hookConfig = this.config.forHook(this.hookName, this.context.hookConfigName);
      var commandList = (hookConfig['command'] || hookConfig['requiredExecutable']).split(/\s+/);
...
HookSigner.prototype.updateSignature = function() {
  var commandResponse = utils.spawnSync(
    'git',
...
HookSigner.prototype.isSignableFile = function(filePath) {
  return (filePath.indexOf('.' + path.sep) === 0 && gitRepo.isTracked(filePath));
};

HookSigner.prototype.hasSignatureChanged = function() {
  return (this.signature() !== this.storedSignature());
};

HookSigner.prototype.signature = function() {
  var hookConfig = _.merge({}, this.config.forHook(this.hookName, this.context.hookConfigName));
...
HookSigner.prototype.hookContents = function() {
  if (!fileUtils.isFile(this.hookPath())) { return ''; }
  return fse.readFileSync(this.hookPath(), 'utf8');
};

HookSigner.prototype.storedSignature = function() {
  var commandResponse = utils.spawnSync('git', ['config', '--local', '--get',  this.signatureConfigKey()]);
...
HookSigner.prototype.signatureConfigKey = function() {
  return ['affiance', this.context.hookConfigName, this.hookName, 'signature'].join('.');
};

module.exports = HookSigner;
