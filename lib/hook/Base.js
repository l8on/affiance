...
module.exports = class HookBase {
  ...
  spawnPromise(command, args, options) {
    return new Promise((resolve, reject) => {
      ...
      this.context.waitForProcessSlot().then((slotId) => {
        ...
        this.context.logger.debug(`${this.hookName()} spawned ${command} process: ${result.pid}`);
  ...
  /**
   * Spawns concurrent commands on chunks of files at a time in order
   * to achieve better performance while limiting the number of
   ...
   * `processCount`, and then invokes the command asynchronously on each
   * chunk of files.
   *
   ...
   * @rejects {Error} an error thrown or emitted during the process run
   */
  spawnConcurrentCommandsOnApplicableFiles() {
